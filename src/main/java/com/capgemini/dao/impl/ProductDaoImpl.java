...
public class ProductDaoImpl extends AbstractDao<ProductEntity, Long> implements ProductDao {
	...
	public Map<String, Integer> findProductsInProgress() {
		...
		List<Tuple> result = query.select(product.name, details.large).from(details)
		...
		return result.stream().collect(Collectors.toMap(p -> p.get(product.name), p -> p.get(details.large)));
	}

	// b
	public List<Long> uniqueBoughtProductsOfClient(Long clientId) {
